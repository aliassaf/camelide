...
let axiom pos s =
  ...
  | Type -> Kind
  | _ ->
    Error.typing_error pos "Cannot type sort %a" print_sort s
...
let rule pos s1 s2 =
  ...
  | _ ->
    Error.typing_error pos "Cannot type (%a, %a) products" print_sort s1 print_sort s2
...
let product_rule s1 s2 pos =
  ...
  | _ -> Error.typing_error pos "This sort of product is not allowed (%a, %a)" print_term s1 print_term s2
...
let rec type_of env term =
  ...
  | Var(x) ->
      ...
      Error.typing_error term.pos "Unbound variable %s" x end end
  | App(t1, t2) ->
...
and check_type env term a =
  ...
  Error.typing_error term.pos "This term has type\n %a\nbut a term was expected of type\n %a" print_term b print_term a

let check_sort env term =
  let s = normalize (type_of env term) in
  match s.body with
  | Sort _ -> ()
  | _ -> Error.typing_error term.pos "This term has an invalid sort %a " print_term s

let check_declaration pos x a =
  if is_declared (Scoping.qualify x)
    then Error.typing_error pos "Declaration %s is already defined" x else
  check_sort [] a
